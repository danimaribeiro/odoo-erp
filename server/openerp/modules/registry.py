...
class Registry(object):
    ...
    def __init__(self, db_name):
        ...
        if openerp.tools.config['unaccent'] and not has_unaccent:
            _logger.warning("The option --unaccent was given but no unaccent() function was found in database; trying to install it now.")
            #
            # Tries to install unaccent function
            #
            try:
                unaccent_cursor = self.db.cursor()
                unaccent_cursor.execute('''
                    BEGIN;
                    CREATE EXTENSION IF NOT EXISTS unaccent;
                    ALTER FUNCTION unaccent(text) IMMUTABLE;
                    COMMIT WORK;
                    ''')
                unaccent_cursor.close()
                has_unaccent = openerp.modules.db.has_unaccent(cr)
            except:
                has_unaccent = openerp.modules.db.has_unaccent(cr)
            ...
            if not has_unaccent:
                ...
                _logger.warning("Installation of unaccent function successfull.")
