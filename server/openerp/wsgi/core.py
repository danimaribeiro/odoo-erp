...
def _decimal_para_float(dic):
    for chave in dic:
        if isinstance(dic[chave], dict):
            dic[chave] = _decimal_para_float(dic[chave])
        elif isinstance(dic[chave], (DecimalOriginal, Decimal)):
            dic[chave] = float(dic[chave])
...
def xmlrpc_return(start_response, service, method, params, legacy_exceptions=False):
    ...
    try:
        ...
        if isinstance(result, dict):
            result = _decimal_para_float(result)
