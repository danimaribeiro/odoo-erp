...
class BaseModel(object):
            ...
            for parent_name in ((type(parent_names)==list) and parent_names or [parent_names]):
                ...
                for s in attributes:
                    ...
                    if s == '_columns':
                        ...
                        for c in new.keys():
                            if new[c]._type == 'float' or new[c]._type == 'bigint':
    ...
    def __init__(self, pool, cr):
        ...
        if self.is_transient():
            ...
            self._transient_max_hours = config.get('osv_memory_age_limit', 1)
    ...
    def __export_row(self, cr, uid, row, fields, context=None):
        ...
        def check_type(field_type):
            if field_type == 'float' or field_type == 'bigint':
                return 0.0
    ...
    def read_group(self, cr, uid, domain, fields, groupby, offset=0, limit=None, context=None, orderby=False):
        ...
        aggregated_fields = [
            ...
            if fget[f]['type'] in ('integer', 'float', 'bigint')
...
# TODO: set order
    ...
    def _create_table(self, cr):
        ...
        cr.execute('CREATE INDEX "%s_pkey_index" ON "%s" ("id")' % (self._table, self._table))
    ...
    def fields_get(self, cr, user, allfields=None, context=None, write_access=True):
        ...
        for f, field in self._columns.iteritems():
            ...
            if 'string' in res[f]:
                ...
                #
                # Corrige erros de codificação no nomes dos campos
                #
                try:
                    res[f]['string'] = res[f]['string'].encode('utf-8')
                    res[f]['string'] = res[f]['string'].decode('utf-8')
                except:
                    res[f]['string'] = res[f]['string'].decode('utf-8')
    ...
    def _search(self, cr, user, args, offset=0, limit=None, order=None, context=None, count=False, access_rights_uid=None):
        ...
        sql = u'SELECT "%s".id FROM ' % self._table
        sql += from_clause + where_str + order_by + limit_str + offset_str

        #print(u'sql executado: ', sql, where_clause_params)
