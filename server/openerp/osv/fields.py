...
class bigint(float):
    ...
    _symbol_f = lambda x: int(x or 0)
    _symbol_set = (_symbol_c, _symbol_f)
    _symbol_get = lambda self, x: int(x or 0)

    def __init__(self, string='unknown', digits=None, digits_compute=None, required=False, **args):
        digits = (18, 0)
        super(bigint, self).__init__(string=string, digits=digits, required=required, **args)
...
#
    def __init__(self, fnct, arg=None, fnct_inv=None, fnct_inv_arg=None, type='float', fnct_search=None, obj=None, store=False, multi=False, **args):
        ...
        if type == 'bigint':
            self._symbol_c = bigint._symbol_c
            self._symbol_f = bigint._symbol_f
            self._symbol_set = bigint._symbol_set
    ...
    def digits_change(self, cr):
        if self._type == 'float' or self._type == 'bigint':
            if self.digits_compute:
                self.digits = self.digits_compute(cr)
            if self.digits:
                precision, scale = self.digits
