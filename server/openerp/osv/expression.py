    ...
    def __leaf_to_sql(self, leaf, table):
        ...
        if leaf == TRUE_LEAF:
            ...
            if isinstance(right, bool):
                ...
                #
                # Integra:
                # Tenta detectar a filtragem de 1 campo many2one baseado em um
                # campo many2many
                #
                if len(right) >= 1:
                    if isinstance(right[0], (list, tuple)) and len(right[0]) == 3:
                        right = right[0][2]
            ...
            if need_wildcard:
                if isinstance(right, str):
                    ...
                    str_utf8 = str(right)
                #
                # Trata os espaços sempre como % nas buscas com like
                #
                params = '%%%s%%' % str_utf8.replace(' ', '%')
