...
def campo_livre(self, boleto):
    ...
    campo_livre = '9'
    campo_livre += str(boleto.beneficiario.codigo_beneficiario.numero).zfill(7)[:7]
...
def agencia_conta(self, boleto):
    return '%s-%s/%s' % (str(boleto.beneficiario.agencia.numero).zfill(4), boleto.beneficiario.agencia.digito, boleto.beneficiario.codigo_beneficiario.numero.zfill(7)[:7])
...
def linha_remessa_400(self, remessa, boleto):
    ...
    #texto += str(boleto.documento.numero).ljust(10)[:10]  ## No Santader, 10 caracteres é muito pouco para o número do documento, então, enviamos o ID
    texto += boleto.identificacao.ljust(10)[:10]
...
def header_retorno_400(self, retorno):
    ...
    beneficiario.codigo_beneficiario.numero = unicode(D(header[110:117]))
