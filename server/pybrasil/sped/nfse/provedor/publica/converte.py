...
def gera_rps(lista_notas, numero_lote, certificado=None):
    ...
    for nota in lista_notas:
        ...
        if nota.prestador.regime_tributario == REGIME_TRIBUTARIO_SIMPLES:
            if nota.natureza_operacao == 0:
                rps.InfRps['NaturezaOperacao'] = '501'
            elif nota.natureza_operacao == 1:
                rps.InfRps['NaturezaOperacao'] = '511'
            else:
                rps.InfRps['NaturezaOperacao'] = '701'
        else:
            if nota.natureza_operacao == 0:
                rps.InfRps['NaturezaOperacao'] = '101'
            elif nota.natureza_operacao == 1:
                rps.InfRps['NaturezaOperacao'] = '111'
            else:
                rps.InfRps['NaturezaOperacao'] = '301'
        ...
        valores['ValorServicos'] = nota.valor.servico or None
        ...
        servico['ItemListaServico'] = nota.servico.codigo.zfill(4)[0:2] + '.' + nota.servico.codigo.zfill(4)[2:]
        ...
        #assinatura = gera_assinatura(URI='R' + str(nota.rps.numero))
        assinatura = gera_assinatura(URI='assinar')
    ...
    #enviarloterpsenvio['__xmlns'] = 'http://www.publica.inf.br'
    #enviarloterpsenvio['__xmlns:xsi'] = 'http://www.w3.org/2001/XMLSchema-instance'
    #enviarloterpsenvio[b'__xsi:schemaLocation'] = b'http://www.publica.inf.br schema_nfse_v03.xsd'
    ...
    raiz['GerarNfseEnvio'] = DicionarioBrasil()
    enviarloterpsenvio = raiz.GerarNfseEnvio
    enviarloterpsenvio['__xmlns'] = 'http://www.publica.inf.br'
    enviarloterpsenvio['Rps'] = rps_raiz.Rps
    
    xml_texto = tira_acentos(raiz.como_xml_em_texto)
    
    ###
    ### Injeta o namespace na assinatura
    ###
    ##tags = [
        ##'Signature',
        ##'SignedInfo', 
    ...
    ##xml_texto = xml_texto.replace('xmlns="http://www.w3.org/2000/09/xmldsig#"', 'xmlns:ds="http://www.w3.org/2000/09/xmldsig#"')
    ##for tag in tags:
       ##xml_texto = xml_texto.replace('<' + tag, '<ds:' + tag)
       ##xml_texto = xml_texto.replace('</' + tag, '</ds:' + tag)
    
    ##xml_texto = xml_texto.replace('<EnviarLoteRpsEnvio>', '<EnviarLoteRpsEnvio xmlns="http://www.publica.inf.br" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.publica.inf.br schema_nfse_v03.xsd">')
    ##print(xml_texto)
    
    open('/home/ari/2015102013841455000181' + str(lote_rps.NumeroLote).zfill(10) + 'rps.xml', 'wb').write(xml_texto)
