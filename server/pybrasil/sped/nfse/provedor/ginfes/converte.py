...
def gera_rps(lista_notas, numero_lote, certificado=None):
    ...
    lote_rps['__Id'] =  str(lote_rps.NumeroLote)
    ...
    for nota in lista_notas:
        ...
        rps.InfRps['__Id'] = str(nota.rps.numero)
    ...
    certificado.doctype = '<!DOCTYPE ns2:EnviarLoteRpsEnvio [<!ATTLIST ns2:LoteRps Id ID #IMPLIED>]>'

    novo_raiz = tira_acentos(raiz.como_xml_em_texto)
    novo_raiz = novo_raiz.replace('<EnviarLoteRpsEnvio', '<EnviarLoteRpsEnvio xmlns:ns2="http://www.ginfes.com.br/servico_enviar_lote_rps_envio_v03.xsd" xmlns:ns0="http://www.ginfes.com.br/tipos_v03.xsd"')

    novo_raiz = novo_raiz.replace('<', '<ns0:')
    novo_raiz = novo_raiz.replace('<ns0:ns0:', '<ns0:')
    novo_raiz = novo_raiz.replace('<ns0:/', '</ns0:')
    novo_raiz = novo_raiz.replace('</ns0:ns0:', '</ns0:')
    novo_raiz = novo_raiz.replace('<ns0:EnviarLoteRpsEnvio', '<ns2:EnviarLoteRpsEnvio')
    novo_raiz = novo_raiz.replace('</ns0:EnviarLoteRpsEnvio', '</ns2:EnviarLoteRpsEnvio')
    novo_raiz = novo_raiz.replace('<ns0:LoteRps', '<ns2:LoteRps')
    novo_raiz = novo_raiz.replace('</ns0:LoteRps', '</ns2:LoteRps')

    novo_raiz = novo_raiz.replace('<ns0:Signature', '<Signature')
    novo_raiz = novo_raiz.replace('</ns0:Signature', '</Signature')

    novo_raiz = novo_raiz.replace('<ns0:SignedInfo', '<SignedInfo')
    novo_raiz = novo_raiz.replace('</ns0:SignedInfo', '</SignedInfo')

    novo_raiz = novo_raiz.replace('<ns0:CanonicalizationMethod', '<CanonicalizationMethod')
    novo_raiz = novo_raiz.replace('</ns0:CanonicalizationMethod', '</CanonicalizationMethod')

    novo_raiz = novo_raiz.replace('<ns0:SignatureMethod', '<SignatureMethod')
    novo_raiz = novo_raiz.replace('</ns0:SignatureMethod', '</SignatureMethod')

    novo_raiz = novo_raiz.replace('<ns0:Reference', '<Reference')
    novo_raiz = novo_raiz.replace('</ns0:Reference', '</Reference')

    novo_raiz = novo_raiz.replace('<ns0:Transforms', '<Transforms')
    novo_raiz = novo_raiz.replace('</ns0:Transforms', '</Transforms')

    novo_raiz = novo_raiz.replace('<ns0:Transform', '<Transform')
    novo_raiz = novo_raiz.replace('</ns0:Transform', '</Transform')

    novo_raiz = novo_raiz.replace('<ns0:DigestMethod', '<DigestMethod')
    novo_raiz = novo_raiz.replace('</ns0:DigestMethod', '</DigestMethod')

    novo_raiz = novo_raiz.replace('<ns0:DigestValue', '<DigestValue')
    novo_raiz = novo_raiz.replace('</ns0:DigestValue', '</DigestValue')

    novo_raiz = novo_raiz.replace('<ns0:SignatureValue', '<SignatureValue')
    novo_raiz = novo_raiz.replace('</ns0:SignatureValue', '</SignatureValue')

    novo_raiz = novo_raiz.replace('<ns0:KeyInfo', '<KeyInfo')
    novo_raiz = novo_raiz.replace('</ns0:KeyInfo', '</KeyInfo')

    novo_raiz = novo_raiz.replace('<ns0:X509Data', '<X509Data')
    novo_raiz = novo_raiz.replace('</ns0:X509Data', '</X509Data')

    novo_raiz = novo_raiz.replace('<ns0:X509Certificate', '<X509Certificate')
    novo_raiz = novo_raiz.replace('</ns0:X509Certificate', '</X509Certificate')

    novo_raiz = novo_raiz.replace('<ns0:?xml version="1.0" encoding="utf-8"?>', '')
...
def gera_consulta_situacao_rps(doc_id, protocolo, prestador, certificado=None):
    raiz = DicionarioBrasil()
    raiz['ConsultarLoteRpsEnvio'] = DicionarioBrasil()
    consultarps = raiz.ConsultarLoteRpsEnvio
    consultarps['__xmlns'] = 'http://www.ginfes.com.br/servico_consultar_lote_rps_envio_v03.xsd'
    consultarps['__Id'] = str(doc_id)
    ...
    novo_raiz = novo_raiz.replace('<Cnpj', '<Cnpj xmlns="http://www.ginfes.com.br/tipos_v03.xsd"')
    novo_raiz = novo_raiz.replace('<InscricaoMunicipal', '<InscricaoMunicipal xmlns="http://www.ginfes.com.br/tipos_v03.xsd"')
