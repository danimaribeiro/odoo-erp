...
class hr_timesheet_line(osv.osv):
    ...
    def _check(self, cr, uid, ids):
        for att in self.browse(cr, uid, ids):
            try:
                if att.sheet_id:
                    if att.sheet_id.state not in ('draft', 'new'):
            ...
            FROM
                ((
                    ...
                    FROM
                        ...
                        #
                        # O ajuste abaixo é para considerar entradas e saídas em datas diferentes
                        # ou várias entradas e saídas no mesmo dia, ajustando todos os cálculos
                        # para o fuso horário de Brasília
                        #
                        """
SELECT -min(a.id) AS id,
cast((a.name AT TIME ZONE 'UTC') AT TIME ZONE 'America/Sao_Paulo' as date) AS name,
s.id AS sheet_id,
0.0 AS total_timesheet,
CASE
WHEN SUM( (CASE WHEN a.action = 'sign_in' THEN (24 * 60) - ((EXTRACT(hour
FROM (a.name AT TIME ZONE 'UTC') AT TIME ZONE 'America/Sao_Paulo') * 60) + EXTRACT(MINUTE
FROM (a.name AT TIME ZONE 'UTC') AT TIME ZONE 'America/Sao_Paulo')) ELSE ((EXTRACT(hour
FROM (a.name AT TIME ZONE 'UTC') AT TIME ZONE 'America/Sao_Paulo') * 60) + EXTRACT(MINUTE
FROM (a.name AT TIME ZONE 'UTC') AT TIME ZONE 'America/Sao_Paulo')) END) ) > (24*60) THEN SUM( (CASE WHEN a.action = 'sign_in' THEN (24 * 60) - ((EXTRACT(hour
FROM (a.name AT TIME ZONE 'UTC') AT TIME ZONE 'America/Sao_Paulo') * 60) + EXTRACT(MINUTE
FROM (a.name AT TIME ZONE 'UTC') AT TIME ZONE 'America/Sao_Paulo')) ELSE ((EXTRACT(hour
FROM (a.name AT TIME ZONE 'UTC') AT TIME ZONE 'America/Sao_Paulo') * 60) + EXTRACT(MINUTE
FROM (a.name AT TIME ZONE 'UTC') AT TIME ZONE 'America/Sao_Paulo')) END) ) - ((24 * 60) * trunc(SUM( (CASE WHEN a.action = 'sign_in' THEN (24 * 60) - ((EXTRACT(hour
FROM (a.name AT TIME ZONE 'UTC') AT TIME ZONE 'America/Sao_Paulo') * 60) + EXTRACT(MINUTE
FROM (a.name AT TIME ZONE 'UTC') AT TIME ZONE 'America/Sao_Paulo')) ELSE ((EXTRACT(hour
FROM (a.name AT TIME ZONE 'UTC') AT TIME ZONE 'America/Sao_Paulo') * 60) + EXTRACT(MINUTE
FROM (a.name AT TIME ZONE 'UTC') AT TIME ZONE 'America/Sao_Paulo')) END) / (24*60))))
ELSE SUM( (CASE WHEN a.action = 'sign_in' THEN (24 * 60) - ((EXTRACT(hour
FROM (a.name AT TIME ZONE 'UTC') AT TIME ZONE 'America/Sao_Paulo') * 60) + EXTRACT(MINUTE
FROM (a.name AT TIME ZONE 'UTC') AT TIME ZONE 'America/Sao_Paulo')) ELSE ((EXTRACT(hour
FROM (a.name AT TIME ZONE 'UTC') AT TIME ZONE 'America/Sao_Paulo') * 60) + EXTRACT(MINUTE
FROM (a.name AT TIME ZONE 'UTC') AT TIME ZONE 'America/Sao_Paulo')) END) )
END AS total_attendance
FROM hr_attendance a
LEFT JOIN (hr_timesheet_sheet_sheet s
LEFT JOIN resource_resource r
LEFT JOIN hr_employee e ON (e.resource_id = r.id) ON (s.user_id = r.user_id)) ON (a.employee_id = e.id
AND s.date_to >= date_trunc('day',(a.name AT TIME ZONE 'UTC') AT TIME ZONE 'America/Sao_Paulo')
AND s.date_from <= (a.name AT TIME ZONE 'UTC') AT TIME ZONE 'America/Sao_Paulo')
WHERE action IN ('sign_in',
'sign_out')
GROUP BY cast((a.name AT TIME ZONE 'UTC') AT TIME ZONE 'America/Sao_Paulo' as date),
s.id
                                                        """ +
