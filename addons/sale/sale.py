...
#            qty = qty - qty % q + q
    ...
    def product_id_change(self, cr, uid, ids, pricelist, product, qty=0,
        ...
        compare_qty = float_compare(float(product_obj.virtual_available) * float(uom2.factor), float(qty) * float(product_obj.uom_id.factor), precision_rounding=product_obj.uom_id.rounding)
        ...
        if not pricelist:
            ...
            valores = self.pool.get('product.pricelist').price_get(cr, uid, [pricelist],
                    product, qty or 1.0, partner_id, dict(context,
                        uom=uom or result.get('product_uom'),
                        date=date_order,
                        ))
            price = valores[pricelist]
            vr_unitario_base = valores['vr_unitario_base'][pricelist]
            if price is False:
                ...
                result.update({'price_unit': price, 'vr_unitario_base': vr_unitario_base, 'price_unit_original': price})
