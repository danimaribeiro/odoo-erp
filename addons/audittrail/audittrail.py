    ...
    def _name_get_resname(self, cr, uid, ids, *args):
        ...
        for resname in self.browse(cr, uid, ids,[]):
            ...
            if model_object and res_id:
                ...
                model_rec_name = model_pool._rec_name
                res = model_pool.read(cr, uid, res_id, [model_rec_name])
                data[resname.id] = res[model_rec_name]
            else:
...
class audittrail_objects_proxy(object_proxy):
    ...
    def get_value_text(self, cr, uid, pool, resource_pool, method, field, value):
        ...
        field_obj = (resource_pool._all_columns.get(field)).column
            ...
            #many_pool = pool.get(field_obj._obj)
            #many_rec_name = many_pool._rec_name
            #print('many_pool', many_pool)
            #print('vaule', value)
            #print('many_rec_name', many_rec_name)
            #data = many_pool.read(cr, uid, value, [many_rec_name])
            #print('data', data)
    ...
    def create_log_line(self, cr, uid, log_id, model, lines=[]):
        ...
        obj_rec_name = obj_pool._rec_name
        model_pool = pool.get('ir.model')
        field_pool = pool.get('ir.model.fields')
        log_line_pool = pool.get('audittrail.log.line')
        for line in lines:
            print('obj_rec_name', obj_rec_name)
            print('obj_pool', obj_pool)
            print('line', line)
