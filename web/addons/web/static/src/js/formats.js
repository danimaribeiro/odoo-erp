...
* Integra
*/
openerp.web.parse_value_price = function(values) {
    if (values == false) {
        return "0,00";
    };

    var value = String(values).split('.');
    var num = value[0];
    var val = String(num).length;
    var tmp = (val % 3 == 0) ? (val / 3) : Math.floor(val / 3) + 1;
    var cont = 1;
    var result = 0;
    var A = [];
    ...
    for ( var i = 1; i <= tmp; i++) {
        result = num % 1000;
        var l = result.toString().length;

        if (tmp > 1 && l < 3 && cont < tmp) {
            var fullfield = "";
            for ( var j = 0; j < (3 - l); j++) {
                fullfield += "0";
            }
            result = fullfield + result;
    ...
    switch (descriptor.widget || descriptor.type) {
        ...
        case 'float_time':
            ...
            if (float_time_pair.length != 2)
                if (value.length <= 2) {
                    float_time_pair = [value, '00']
                }
                else {
                    float_time_pair = [value.slice(-4, -2), value.slice(-2)]
                }
            ...
            //if ((hours > 23) || (hours < 0)) throw new Error(str(hours) + " não é uma hora válida");;
            if ((minutes > 60) || (minutes < 0)) throw new Error(str(minutes) + " não é um minuto válido");
        ...
        case 'date':
            var date = Date.parseExact(value + ' 12:00:00', date_pattern + ' ' + time_pattern);
            if (date !== null)
                return openerp.web.date_to_str(date);
            date = Date.parse(value + ' 12:00:00');
            if (date !== null)
                return openerp.web.date_to_str(date);
