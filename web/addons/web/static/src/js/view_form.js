openerp.web.form = function(openerp) {
    ...
    var _t = openerp.web._t, _lt = openerp.web._lt;
    ...
    var historicdigits = new Array();
    var plano_conta_digits = new Array();
    /*
     * Alterado Integra --- FIM
     */

    openerp.web.views.add('form', 'openerp.web.FormView');
    openerp.web.FormView = openerp.web.View
            .extend( /** @lends openerp.web.FormView# */
            {
                /**
                ...
                init_view : function() {
                    if (this.embedded_view) {
                        ...
                        $.async_when().then(function() {
                            def.resolve(self.embedded_view);
                        });
                        return def.promise();
                    } else {
                ...
                on_loaded : function(data) {
                    ...
                    if (data) {
                        ...
                        var frame = new (this.registry.get_object('frame'))(
                                this, this.fields_view.arch);

                        this.rendered = QWeb.render(this.form_template, {
                            'frame' : frame,
                            'widget' : this
                        });
                    }
                    ...
                    _.each(this.widgets, function(w) {
                        w.start();
                        $(w).bind('widget-focus.formBlur',
                                self.proxy('widgetFocused')).bind(
                                'widget-blur.formBlur',
                                self.proxy('widgetBlurred'));
                    });
                    this.$form_header = this.$element
                            .find('.oe_form_header:first');
                    this.$form_header.find(
                            'div.oe_form_pager button[data-pager-action]')
                            .click(function() {
                                var action = $(this).data('pager-action');
                                self.on_pager_action(action);
                            });

                    this.$form_header.find('button.oe_form_button_save').click(
                            this.on_button_save);
                    this.$form_header.find('button.oe_form_button_cancel')
                            .click(this.on_button_cancel);

                    if (!this.sidebar && this.options.sidebar
                            && this.options.sidebar_id) {
                        this.sidebar = new openerp.web.Sidebar(this,
                                this.options.sidebar_id);
                        this.sidebar.start();
                        this.sidebar.do_unfold();
                        this.sidebar.attachments = new openerp.web.form.SidebarAttachments(
                                this.sidebar, this);
                ...
                do_show : function() {
                    ...
                    return this.has_been_loaded.pipe(function() {
                        ...
                        if (self.dataset.index === null || self.dataset.index < 0) {
                            ...
                            result = self.dataset.read_index(
                ...
                on_record_loaded : function(record) {
                    ...
                    return $.when.apply(null, set_values).pipe(function() {
                        if (!record.id) {
                            // New record: Second pass in order to trigger the
                            // onchanges
                            ...
                            _.each(self.fields_order, function(field_name) {
                                if (record[field_name]) {
                                    ...
                                    //if (this.desenhado || field.required) {
                                        field.dirty = true;
                ...
                do_update_pager : function(hide_index) {
                    ...
                    $pager.find('button').prop('disabled',
                            this.dataset.ids.length < 2);
                    $pager.find('span.oe_pager_index').html(index);
                    $pager.find('span.oe_pager_count').html(
                ...
                parse_on_change : function(on_change, widget) {
                    ...
                    var argument_replacement = {
                        ...
                        'context' : function(i) {
                            context_index = i;
                            var ctx = new openerp.web.CompoundContext(
                                    self.dataset.get_context(), widget
                            ...
                            .map(
                                    ...
                                    function(a, i) {
                                        ...
                                        if (splitted.length > 1
                                                && _.str.trim(splitted[0]) === "parent"
                                                && self.dataset.parent_view) {
                                            if (parent_fields === null) {
                                                parent_fields = self.dataset.parent_view
                                                        .get_fields_values([ self.dataset.child_name ]);
                                            }
                                            var p_val = parent_fields[_.str
                                                    .trim(splitted[1])];
                                            if (p_val !== undefined) {
                                                return p_val == null ? false
                                                        : p_val;
                                            }
                                        }
                                        // string literal
                ...
                do_onchange : function(widget, processed) {
                    var self = this;
                    return this.on_change_mutex
                            .exec(function() {
                                try {
                                    var response = {}, can_process_onchange = $
                                            .Deferred();
                                    processed = processed || [];
                                    processed.push(widget.name);
                                    var on_change = widget.node.attrs.on_change;
                                    if (on_change) {
                                        var change_spec = self.parse_on_change(
                                                on_change, widget);
                                        if (change_spec) {
                                            var ajax = {
                                            ...
                                            can_process_onchange = self
                                                    .rpc(
                                                            ajax,
                                                            {
                                                                model : self.dataset.model,
                                                                method : change_spec.method,
                                                                args : [ (self.datarecord.id == null ? []
                                                                        : [ self.datarecord.id ]) ]
                                                                        .concat(change_spec.args),
                                                                context_id : change_spec.context_index == undefined ? null
                                                                        : change_spec.context_index + 1
                                                            })
                                                    .then(function(r) {
                                                        _.extend(response, r);
                                                    });
                    ...
                    try {
                        ...
                        if (result.value) {
                            for ( var f in result.value) {
                                if (!result.value.hasOwnProperty(f)) {
                                    continue;
                                }
                                var field = this.fields[f];
                                // If field is not defined in the view, just
                                // ignore it
                                if (field) {
                        ...
                        if (!_.isEmpty(result.warning)) {
                            $(
                                    QWeb.render("CrashManagerWarning",
                ...
                on_button_new : function() {
                    var self = this;
                    var def = $.Deferred();
                    $.when(this.has_been_loaded).then(
                            function() {
                                if (self.can_be_discarded()) {
                                    var keys = _.keys(self.fields_view.fields);
                                    if (keys.length) {
                                        self.dataset.default_get(keys).pipe(
                                                self.on_record_loaded).then(
                                                function() {
                                                    def.resolve();
                                                });
                                    } else {
                ...
                can_be_discarded : function() {
                    return !this.$element.is('.oe_form_dirty')
                            || confirm(_t("Warning, the record has been modified, your changes will be discarded."));
                },
                /**
                 ...
                 *
                 * @param {Function}
                 *            [success] callback on save success
                 * @param {Boolean}
                 *            [prepend_on_create=false] if ``do_save`` creates a
                 ...
                 *            r
                 * @param {Function}
                 *            success callback to execute after having updated
                 *            the dataset
                 * @param {Boolean}
                ...
                reload : function() {
                    ...
                    return this.reload_mutex.exec(function() {
                        if (self.dataset.index == null
                            ...
                            return self.dataset.read_index(
                                    _.keys(self.fields_view.fields), {
                                        context : {
                                            'bin_size' : true
                                        }
                ...
                is_interactible_record : function() {
                    ...
                    if (!id) {
                        ...
                        if (openerp.web.BufferedDataSet.virtual_id_regex
                                .test(id))
                            return false;
                    }
                    return true;
                ...
                sidebar_context : function() {
                    return this.do_save().pipe(_.bind(function() {
                        return this.get_fields_values();
                    }, this));
                },
                open_defaults_dialog : function() {
                    ...
                    var fields = _
                            ...
                            .map(
                                    function(field, name) {
                                        ...
                                        // ignore fields which are empty,
                                        // invisible, readonly, o2m
                                        // or m2m
                                        if (!value
                                                || field.invisible
                                                || field.readonly
                    ...
                    var conditions = _.chain(fields).filter(function(field) {
                        return field.change_default;
                    }).value();

                    var d = new openerp.web.Dialog(
                            this,
                            {
                                ...
                                buttons : [
                                        ...
                                        },
                                        {
                                            ...
                                            click : function() {
                                                var $defaults = d.$element
                                                        .find('#formview_default_fields');
                                                var field_to_set = $defaults
                                                        .val();
                                                if (!field_to_set) {
                                                    $defaults
                                                            .parent()
                                                ...
                                                var condition = d.$element
                                                        .find(
                                                                '#formview_default_conditions')
                                                        .val(), all_users = d.$element
                                                        .find(
                                                                '#formview_default_all')
                                                        .is(':checked');
                                                new openerp.web.DataSet(self,
                                                        'ir.values')
                                                        .call(
                                                                'set_default',
                                                                [
    ...
    openerp.web.FormDialog = openerp.web.Dialog.extend({
        ...
        start : function() {
            this._super();
            this.form = new openerp.web.FormView(this, this.dataset,
                    this.view_id, {
                        sidebar : false,
                        pager : false
                    });
    ...
    openerp.web.form.SidebarAttachments = openerp.web.OldWidget
            .extend({
                init : function(parent, form_view) {
                    var $section = parent.add_section(_t('Attachments'),
                            'attachments');
                    ...
                    this.attachment_title = function(name) {
                        return _.str.sprintf(_t("Delete the attachment %s"),
                                name);
                    };
                },
                do_update : function() {
                    if (!this.view.datarecord.id) {
                        ...
                        (new openerp.web.DataSetSearch(this, 'ir.attachment',
                                this.view.dataset.get_context(), [
                                        ...
                                        [ 'type', 'in', [ 'binary', 'url' ] ] ]))
                                .read_slice([ 'name', 'url', 'type' ], {})
                                .then(this.on_attachments_loaded);
                    }
                },
                on_attachments_loaded : function(attachments) {
                    this.attachments = attachments;
                    this.$div.html(QWeb.render('FormView.sidebar.attachments',
                            this));
                    this.$element.find('.oe-binary-file').change(
                            this.on_attachment_changed);
                    this.$element.find('.oe-sidebar-attachment-delete').click(
                            this.on_attachment_delete);
                },
                on_attachment_changed : function(e) {
                    ...
                    if ($e.val() != '') {
                        ...
                        $e.parent().find('input[type=file]').prop('disabled',
                                true);
                        $e.parent().find('button').prop('disabled', true).find(
                                'img, span').toggle();
                    }
                },
                on_attachment_delete : function(e) {
                    var self = this, $e = $(e.currentTarget);
                    var name = _.str.trim($e.parent().find(
                            'a.oe-sidebar-attachments-link').text());
                    if (confirm(_.str
                            .sprintf(
                                    _t("Do you really want to delete the attachment %s?"),
                                    name))) {
                        this
                                .rpc(
                                        '/web/dataset/unlink',
                                        ...
                                        function(r) {
                                            ...
                                            self
                                                    .do_notify(
                                                            ...
                                                            _.str
                                                                    .sprintf(
                                                                            _t("The attachment '%s' has been deleted"),
                                                                            name));
                                        });
                    }
                }
    ...
    openerp.web.form.compute_domain = function(expr, fields) {
        ...
        for ( var i = expr.length - 1; i >= 0; i--) {
            ...
            if (ex.length == 1) {
                ...
                default:
                    throw new Error(_.str.sprintf(
                            _t("Unknown operator %s in domain %s"), ex, JSON
                                    .stringify(expr)));
                }
            }
            ...
            var field_value = field.get_value ? fields[ex[0]].get_value()
                    : fields[ex[0]].value;
            var op = ex[1];
            var val = ex[2];
            ...
            case 'not in':
                ...
                break;
            default:
                console.warn(_t("Unsupported operator %s in domain %s"), op,
                        JSON.stringify(expr));
            }
        }
    ...
    openerp.web.form.Widget = openerp.web.OldWidget
            .extend(/** @lends openerp.web.form.Widget# */
            {
                template : 'Widget',
                /**
                ...
                init : function(view, node) {
                    ...
                    this.element_class = [ 'formview', this.view.view_id,
                            this.element_name, this.view.widgets_counter++ ]
                            .join("_");
                ...
                process_modifiers : function() {
                    ...
                    for ( var a in this.modifiers) {
                        this[a] = compute_domain(this.modifiers[a],
                                this.view.fields);
                    }
                },
                update_dom : function() {
                ...
                _build_view_fields_values : function(blacklist) {
                    ...
                    if (a_dataset.parent_view) {
                        fields_values.parent = a_dataset.parent_view
                                .get_fields_values([ a_dataset.child_name ]);
                    }
                    return fields_values;
                },
                _build_eval_context : function(blacklist) {
                    var a_dataset = this.view.dataset;
                    return new openerp.web.CompoundContext(a_dataset
                            .get_context(), this
                            ._build_view_fields_values(blacklist));
                },
                /**
                ...
                build_context : function(blacklist) {
                    ...
                    if (v_context.__ref || true) { // TODO: remove true
                        ...
                        v_context = new openerp.web.CompoundContext(v_context)
                ...
                build_domain : function() {
                    ...
                    if (!(final_domain instanceof Array) || true) { // TODO:
                        ...
                        final_domain = new openerp.web.CompoundDomain(
    ...
    openerp.web.form.WidgetFrame = openerp.web.form.Widget
            .extend({
                ...
                add_row : function() {
                    if (this.table.length) {
                        this
                                .set_row_cells_with(this.table[this.table.length - 1]);
                    }
                    var row = [];
                    this.table.push(row);
                    this.x = 0;
                ...
                set_row_cells_with : function(row) {
                    var bypass = 0, max_width = 100, row_length = row.length;
                    ...
                    var size_unit = Math.round(max_width
                            / (this.columns - bypass)), colspan_sum = 0;
                    for ( var i = 0; i < row.length; i++) {
                        var w = row[i];
                        ...
                        if (w.width === undefined) {
                            var width = (i === row_length - 1 && colspan_sum === this.columns) ? max_width
                                    : Math.round(size_unit * w.colspan);
                            max_width -= width;
                            w.width = width + '%';
                        }
                    }
                ...
                handle_node : function(node) {
                    ...
                    if (node.tag == 'field') {
                        ...
                        if (node.attrs.widget == 'statusbar'
                            ...
                            // This way we can retain backward compatibility
                            // between addons and old clients
                            node.attrs.colspan = (parseInt(node.attrs.colspan,
                                    10) || 1) + 1;
                            node.attrs.nolabel = '1';
                        }
                    }
                    var widget = new (this.view.registry.get_any([
                            node.attrs.widget, type.type, node.tag ]))(
                            this.view, node);
                    if (node.tag == 'field') {
                        if (!this.view.default_focus_field
                                || node.attrs.default_focus == '1') {
                            this.view.default_focus_field = widget;
                        }
                        if (node.attrs.nolabel != '1') {
                            var label = new (this.view.registry
                                    .get_object('label'))(this.view, node);
                            label["for"] = widget;
    ...
    openerp.web.form.WidgetNotebook = openerp.web.form.Widget
            .extend({
                ...
                init : function(view, node) {
                    ...
                    for ( var i = 0; i < node.children.length; i++) {
                        var n = node.children[i];
                        if (n.tag == "page") {
                            var page = new (this.view.registry
                                    .get_object('notebookpage'))(this.view, n,
                                    this, this.pages.length);
                ...
                start : function() {
                    var self = this;
                    this._super.apply(this, arguments);
                    this.$element.find('> ul > li').each(
                            function(index, tab_li) {
                                var page = self.pages[index], id = _
                                        .uniqueId(self.element_name + '-');
                                page.element_id = id;
                    ...
                    this.$element.tabs();
                    this.view.on_button_new
                            .add_first(this.do_select_first_visible_tab);
    ...
    openerp.web.form.WidgetNotebookPage = openerp.web.form.WidgetFrame.extend({
        ...
        start : function() {
            this._super.apply(this, arguments);
            this.$element_tab = this.notebook.$element.find('> ul > li:eq('
                    + this.index + ')');
        },
        update_dom : function() {
            if (this.invisible
                    && this.index === this.notebook.$element.tabs('option',
                            'selected')) {
    ...
    openerp.web.form.WidgetButton = openerp.web.form.Widget
            .extend({
                ...
                on_confirmed : function() {
                    ...
                    return this.view.do_execute_action(_.extend({},
                            this.node.attrs, {
                                context : context
                ...
                check_disable : function() {
                    var disabled = (this.readonly || this.force_disabled || !this.view
    ...
    openerp.web.form.WidgetLabel = openerp.web.form.Widget
            .extend({
                ...
                init : function(view, node) {
                    ...
                    if (this.node.tag == 'label' && !this.string
                            && this.node.children.length) {
                        this.string = this.node.children[0];
                        this.align = 'left';
                    }

                    if (this.node.tag == 'label'
                            && (this.align === 'left'
                                    || this.node.attrs.colspan || (this.string && this.string.length > 32))) {
                        this.template = "WidgetParagraph";
                        this.colspan = parseInt(this.node.attrs.colspan || 1,
                                10);
                        // Widgets default to right-aligned, but paragraph
                        // defaults to
                        // left-aligned
                        ...
                        this.multilines = this.string
                                && _.str.lines(this.string).length > 1;
                    } else {
                        this.colspan = 1;
                        this.width = '1%';
                ...
                render : function() {
                    ...
                    // Actual label widgets should not have a false and have
                    // type label
                    return QWeb.render(this.template, {
                        widget : this
                    });
                ...
                start : function() {
                    ...
                    var self = this;
                    if (this['for']
                            && (this['for'].help || openerp.connection.debug)) {
    ...
    openerp.web.form.Field = openerp.web.form.Widget.extend(/** @lends openerp.web.form.Field# */
    {
        ...
        init : function(view, node) {
            ...
            this.type = node.attrs.widget
                    || view.fields_view.fields[node.attrs.name].type;
        ...
        start : function() {
            ...
            if (this.field.translate) {
                ...
                this.$element.find('.oe_field_translate').click(
                        this.on_translate);
            }
        ...
        update_dom : function(show_invalid) {
            this._super.apply(this, arguments);
            if (this.field.translate) {
                this.$element.find('.oe_field_translate').toggle(
                        !!this.view.datarecord.id);
            }
    ...
    openerp.web.form.FieldChar = openerp.web.form.Field.extend({
        ...
        set_value : function(value) {
            ...
            var input_element = this.$element.find('input');
            if ($(input_element).attr("name") === "formato_conta") {
                var array = input_element.val().split(".");
                plano_conta_digits = array;
            }
        ...
        set_value_from_ui : function() {
            this.value = openerp.web.parse_value(this.$element.find('input')
                    .val(), this);
            this._super();
        },
        validate : function() {
            this.invalid = false;
            try {
                var value = openerp.web.parse_value(this.$element.find('input')
                        .val(), this, '');
    ...
    // Alterado Integra
    //
    openerp.web.form.FieldFloat = openerp.web.form.FieldChar
            .extend({
                //
                // Alterado Integra
                //
                ...
                set_value : function(value) {
                    ...
                    if (this.element_name === 'field_precisao_anterior_separador_integer')
                        historicdigits.push(value);
                    if (this.element_name === 'field_precisao_posterior_separador_integer')
                        historicdigits.push(value);
    ...
    // Alterado Integra
    //
    openerp.web.form.FieldReadOnlyWidget = openerp.web.form.FieldChar.extend({
    ...
    openerp.web.form.PlanoContaFormatWidget = openerp.web.form.FieldChar
            .extend({
                init : function(field_manager, node) {
                    this._super(field_manager, node);
                },
                start : function() {
                    this._super.apply(this, arguments);
                    this.$input = this.$element.find('input');
                    this.$input.blur(this.on_ui_change);
                },
                set_value : function(value) {
                    this._super.apply(this, arguments);
                    this.value = value;

                    if (plano_conta_digits.length > 1) {
                        options.digits = new Array();
                        options.digits = plano_conta_digits;
                        historicdigits = new Array();
                    }
                    this.$input.planocontaformat(options);
                    this._super.apply(this, arguments);
                },
                set_value_from_ui : function() {
    ...
    openerp.web.form.HistoricFormatWidget = openerp.web.form.FieldChar.extend({
        init : function(field_manager, node) {
            this._super(field_manager, node);
        },
        start : function() {
        ...
        set_value : function(value) {
            this._super.apply(this, arguments);
            this.value = value;
            if (historicdigits.length > 1) {
                options.digits = new Array();
                options.digits = historicdigits;
                historicdigits = new Array();
            }
            this.$input.historicformat(options);
            this._super.apply(this, arguments);
        },
        set_value_from_ui : function() {
            var value = this.$input.val() || false;
    ...
    openerp.web.form.PriceFormatWidget = openerp.web.form.FieldFloat.extend({
        type_of_field : "float",
        init : function(field_manager, node) {
            this._super(field_manager, node);
        ...
        start : function() {
            this._super.apply(this, arguments);
            this.$input = this.$element.find('input.field_priceformat');
        ...
        set_value : function(value) {
            ...
            this._super.apply(this, [ value ]);
            var val = openerp.web.parse_value_price(value);
            if (val === undefined)
                val = "0,00";
            this.$input.val(val);
        },
        parse_client : function(v) {
            return openerp.web.parse_value(v, {
                "widget" : this.type_of_field
            });
        },
        set_value_from_ui : function() {
            var value = this.$input.val() || false;
        ...
        get_value : function() {
            return this.parse_client(this.value);
        }
    });
    ...
    openerp.web.DateTimeWidget = openerp.web.OldWidget.extend({
        ...
        start : function() {
            ...
            this.$input_picker = this.$element
            ...
            this.$element.find('img.oe_datepicker_trigger').click(
                    function() {
                        if (self.readonly
                        ...
                        self.picker('setDate', self.value ? openerp.web
                                .auto_str_to_date(self.value) : new Date());
        ...
        set_readonly : function(readonly) {
            ...
            this.$element.find('img.oe_datepicker_trigger').toggleClass(
        ...
        format_client : function(v) {
            return openerp.web.format_value(v, {
                "widget" : this.type_of_date
            });
        },
        on_change : function() {
    ...
    openerp.web.form.FieldText = openerp.web.form.Field.extend({
        ...
        set_value_from_ui : function() {
            this.value = openerp.web.parse_value(this.$element.find('textarea')
                    .val(), this);
            this._super();
        },
        validate : function() {
            this.invalid = false;
            try {
                var value = openerp.web.parse_value(this.$element.find(
                        'textarea').val(), this, '');
                this.invalid = this.required && value === '';
            } catch (e) {
                this.invalid = true;
        ...
        do_resize : function(max_height) {
            ...
            var $input = this.$element.find('textarea'), $div = $(
                    '<div style="position: absolute; z-index: 1000; top: 0"/>')
                    .width($input.width()), new_height;
            $div.text($input.val());
            _.each('font-family,font-size,white-space'.split(','), function(
                    style) {
    ...
    openerp.web.form.FieldProgressBar = openerp.web.form.Field.extend({
        ...
        set_value : function(value) {
            ...
            var formatted_value = openerp.web.format_value(show_value, {
                type : 'float'
            }, '0');
            this.$element.find('div')
                    .progressbar('option', 'value', show_value).find('span')
    ...
    openerp.web.form.FieldSelection = openerp.web.form.Field
            .extend({
                ...
                start : function() {
                    ...
                    $select.change(this.on_ui_change).change(function() {
                        ischanging = true;
                    }).click(function() {
                        ischanging = false;
                    }).keyup(function(e) {
    ...
    openerp.web.form.FieldMany2One = openerp.web.form.Field
            .extend({
                ...
                start : function() {
                    ...
                    var self = this;
                    this.$input = this.$element.find("input");
                    this.$drop_down = this.$element
                            .find(".oe-m2o-drop-down-button");
                    this.$menu_btn = this.$element.find(".oe-m2o-cm-button");
                    var $self = $(self), ignore_blur = false, cm_ignore_blur = false;

                    // context menu
                    var init_context_menu_def = $
                            .Deferred()
                            .then(
                                    function(e) {
                                        var rdataset = new openerp.web.DataSetStatic(
                                                self, "ir.values", self
                                                        .build_context());
                                        rdataset
                                                .call(
                                                ...
                                                .then(
                                                        function(result) {
                                                            ...
                                                            bindings[self.cm_id
                                                                ...
                                                                var pop = new openerp.web.form.FormOpenPopup(
                                                                        self.view);
                                                                pop
                                                                        .show_element(
                                                                                self.field.relation,
                                                                ...
                                                                pop.on_write_completed
                                                                        .add_last(function() {
                                                                            self
                                                                                    .set_value(self.value[0]);
                                                                            self
                                                                                    .focus();
                                                                        });
                                                            };
                                                            _
                                                                    .each(
                                                                            _
                                                                                    .range(self.related_entries.length),
                                                                            function(
                                                                                    i) {
                                                                                bindings[self.cm_id
                                                                                        + "_related_"
                                                                                        + i] = function() {
                                                                                    self
                                                                                            .open_related(self.related_entries[i]);
                                                                                };
                                                                            });
                                                            var cmenu = self.$menu_btn
                                                                    .contextMenu(
                                                                            self.cm_id,
                                                                            {
                                                                                'noRightClick' : true,
                            ...
                            .keyup(function() {
                                if (self.$input.val() === "") {
                                    self._change_int_value(null);
                                        ...
                                        || (self.value && self.$input.val() !== self.value[1])) {
                    ...
                    this.$input.closest(".ui-dialog").bind(
                            "dialogdragstart dialogresizestart", function() {
                                self.$input.autocomplete("close");
                            });

                    var anyoneLoosesFocus = function() {
                        if (!self.$input.is(":focus")
                                && !self.$input.autocomplete("widget").is(
                                        ":visible") && !self.value) {
                    ...
                    this.$input
                            .bind({
                                ...
                                blur : function() {
                                    // autocomplete open
                                    if (ignore_blur || cm_ignore_blur) {
                                        return;
                                    }
                                    var child_popup_open = _(
                    ...
                    dataset
                            .name_search(
                                    ...
                                    function(data) {
                                        ...
                                        // search more... if more results that
                                        // max
                                        if (values.length > self.limit) {
                                            var open_search_popup = function(
                                                    data) {
                                                self._change_int_value(null);
                                        ...
                                        // quick create
                                        var raw_result = _(data.result).map(
                                                function(x) {
                                                    return x[1];
                                                });
                                        if (search_val.length > 0
                                                && !_.include(raw_result,
                                                        search_val)
                                            ...
                                            values
                                                    .push({
                                                        label : _.str
                                                                .sprintf(
                                                                        _t('<em>   Create "<strong>%s</strong>"</em>'),
                                                                        $(
                                                                                '<span />')
                                                                                .text(
                                                                                        search_val)
                                        ...
                                        values
                                                .push({
                                                    label : _t("<em>   Create and Edit...</em>"),
                                                    action : function() {
                                                        self
                                                                ._change_int_value(null);
                                                        self
                                                                ._search_create_popup(
                                                                        "form",
                                                                        undefined,
                                                                        {
                                                                            "default_name" : search_val
                                                                        });
                                                    }
                                                });
                ...
                _quick_create : function(name) {
                    ...
                    if (self.get_definition_options().quick_create === undefined
                            || self.get_definition_options().quick_create) {
                        var dataset = new openerp.web.DataSetStatic(this,
                                this.field.relation, self.build_context());
                ...
                _search_create_popup : function(view, ids, context) {
                    ...
                    }, self.build_domain(), new openerp.web.CompoundContext(
                            self.build_context(), context || {}));
                    pop.on_select_elements.add(function(element_ids) {
                        var dataset = new openerp.web.DataSetStatic(self,
                ...
                _change_int_value : function(value) {
                    ...
                    if (back_orig_value === undefined) { // first use after a
                        // set_value()
                        return;
                    }
                    if (this.value !== undefined
                            && ((back_orig_value ? back_orig_value[0] : null) !== (this.value ? this.value[0]
                                    : null))) {
                        this.on_ui_change();
                    }
                },
                set_value : function(value) {
                    ...
                    if (value && !(value instanceof Array)) {
                        // name_get in a m2o does not use the context of the
                        // field
                        var dataset = new openerp.web.DataSetStatic(this,
                                this.field.relation, self.view.dataset
                                        .get_context());
                ...
                get_value : function() {
                    ...
                    if (this.value === undefined)
                        return this.original_value ? this.original_value[0]
                                : false;
                    return this.value ? this.value[0] : false;
                },
                validate : function() {
                    this.invalid = false;
                    var val = this.tmp_value !== undefined ? this.tmp_value
                            : this.value;
                    if (val === null) {
    ...
    openerp.web.form.FieldOne2Many = openerp.web.form.Field
            .extend({
                ...
                start : function() {
                    ...
                    this.dataset = new openerp.web.form.One2ManyDataSet(this,
                            this.field.relation);
                ...
                load_views : function() {
                    ...
                    this.viewmanager = new openerp.web.ViewManager(this,
                            this.dataset, views, {});
                    ...
                    this.viewmanager.on_controller_inited.add_last(function(
                            view_type, controller) {
                        if (view_type == "list") {
                            controller.o2m = self;
                            if (self.is_readonly())
                            ...
                            if (view_type == 'page' || self.is_readonly()) {
                                $(".oe_form_buttons", controller.$element)
                                        .children().remove();
                            }
                    ...
                    this.viewmanager.on_mode_switch.add_first(function(n_mode,
                            b, c, d, e) {
                ...
                reload_current_view : function() {
                    var self = this;
                    return self.is_loaded = self.is_loaded
                            .pipe(
                                    function() {
                                        ...
                                        if (active_view === "list") {
                                            ...
                                            self.form_last_update = self.form_last_update
                                                    .pipe(act, act);
                                            return self.form_last_update;
                                        } else if (active_view === "graph") {
                                            return view.do_search(self
                                                    .build_domain(),
                ...
                get_value : function() {
                    ...
                    return val.concat(_.map(this.dataset.to_delete,
                            function(x) {
                                                    ...
                                                    if (this.viewmanager.active_view === "form") {
                                                        ...
                                                        if (!res.isResolved()
                                                                && !res
                                                                        .isRejected()) {
                                                            console
                                                                    .warn("Asynchronous get_value() is not supported in form view.");
                                                        }
                                                        return res;
                                                        ...
                                                        if (!res.isResolved()
                                                                && !res
                                                                        .isRejected()) {
                                                            console
                                                                    .warn("Asynchronous get_value() is not supported in list view.");
                                                        }
                                                        return res;
                                                    }
                                                }
                ...
                is_valid : function() {
                    ...
                    case 'form':
                        return _(view.fields).chain().invoke('is_valid').all(
                                _.identity).value();
                        break;
                    case 'list':
                    ...
                    if (this.previous_readonly !== this.readonly) {
                        ...
                        if (this.viewmanager) {
                            this.is_loaded = this.is_loaded.pipe(function() {
                                ...
                                return $.when(self.load_views()).then(
                                        function() {
                                            self.reload_current_view();
                                        });
                            });
    ...
    openerp.web.form.One2ManyListView = openerp.web.ListView
            .extend({
                ...
                is_valid : function() {
                    ...
                    // If the form has not been modified, the view can only be
                    // valid
                    // NB: is_dirty will also be set on
                    // defaults/onchanges/whatever?
                ...
                first_edition_form : function() {
                    var get_form = function(group_or_list) {
                        ...
                        return _(group_or_list.children).chain().map(get_form)
                                .compact().first().value();
                    };
                        ...
                        pop
                                .select_element(
                                        self.o2m.field.relation,
                                        {
                                            ...
                                            initial_view : "form",
                                            alternative_form_view : self.o2m.field.views ? self.o2m.field.views["form"]
                                                    : undefined,
                                            create_function : function(data,
                                                    callback, error_callback) {
                                                return self.o2m.dataset
                                                        .create(data)
                                                        .then(
                                                                function(r) {
                                                                    self.o2m.dataset
                                            ...
                                            read_function : function() {
                                                return self.o2m.dataset.read_ids
                                                        .apply(
                                                                self.o2m.dataset,
                                                                arguments);
                                            },
                                        ...
                                        }, self.o2m.build_domain(), self.o2m
                                                .build_context());
    ...
    openerp.web.form.One2ManyList = openerp.web.ListView.List
            .extend({
                KEY_RETURN : 13,
                // blurring caused by hitting the [Return] key, should skip the
                // autosave-on-blur and let the handler for [Return] do its
                // thing
                __return_blur : false,
                render_row_as_form : function() {
                    ...
                    return this._super
                            ...
                            .then(
                                    function() {
                                        // Replace the "Save Row" button with
                                        // "Cancel Edition"
                                        self.edition_form.$element
                                                ...
                                                .delegate(
                                                        ...
                                                        function() {
                                                            self
                                                                    .cancel_pending_edition();
                                                        });

                                        // Replace form's do_execute_action by a
                                        // call to listview's:
                                        // because the form view is actually
                                        // destroyed at one point, the
                                        // RPC request to load&execute the
                                        // action may get dropped on the
                                        // floor in some cases (core.js:1173)
                                        // leading to the flow not
                                        // terminating for complex cases (a
                                        // button of type action, which
                                        // needs 2 or 3 RPC roundtrip to execute
                                        // the action fully).
                                        // => because do_execute_action really
                                        // is on View, use the
                                        // listview's
                                        self.edition_form.do_execute_action = function(
                                                action, dataset, record_id,
                                                _callback) {
                                            return self.view
                                                    .do_execute_action(
                                                            action,
                                                            ...
                                                            function() {
                                                                self.view
                                                                        .reload_record(self.view.records
                                                                                .get(record_id));
                                                            });
                                        };
    ...
    openerp.web.form.WidgetOnlyEdit = openerp.web.form.FieldFloat.extend({
        init : function(view, node) {
            this._super(view, node);
        },
        start : function() {
            this._super.apply(this, arguments);
            $(".oe_form_button_create").css("display", "none");
            $(".oe_form_button_duplicate").css("display", "none");
            $(".oe_form_button_delete").css("display", "none");
    ...
    openerp.web.form.One2ManyFormView = openerp.web.FormView.extend({
        ...
        on_loaded : function(data) {
            this._super(data);
            var self = this;
            this.$form_header.find('button.oe_form_button_create').click(
                    function() {
    ...
    openerp.web.form.FieldMany2Many = openerp.web.form.Field.extend({
        ...
        start : function() {
            ...
            this.dataset = new openerp.web.form.Many2ManyDataSet(this,
                    this.field.relation);
        ...
        load_view : function() {
            var self = this;
            this.list_view = new openerp.web.form.Many2ManyListView(this,
                    this.dataset, false, {
    ...
    openerp.web.form.Many2ManyListView = openerp.web.ListView.extend(/** @lends openerp.web.form.Many2ManyListView# */
    {
        do_add_record : function() {
            ...
            }, new openerp.web.CompoundDomain(this.m2m_field.build_domain(), [
            ...
            pop.on_select_elements.add(function(element_ids) {
                _.each(element_ids, function(element_id) {
                    if (!_.detect(self.dataset.ids, function(x) {
                        ...
                        self.dataset.set_ids([].concat(self.dataset.ids,
                                [ element_id ]));
     ...
     * @extends openerp.web.OldWidget
     */
    openerp.web.form.SelectCreatePopup = openerp.web.OldWidget
            .extend(/** @lends openerp.web.form.SelectCreatePopup# */
            {
                template : "SelectCreatePopup",
                /**
                 * options: - initial_ids - initial_view: form or search
                 * (default search) - disable_multiple_selection -
                 * alternative_form_view - create_function (defaults to a naive
                 * saving behavior) - parent_view - child_name -
                 * form_view_options - list_view_options - read_function
                 */
                ...
                start : function() {
                    this._super();
                    var self = this;
                    this.dataset = new openerp.web.ProxyDataSet(this,
                            this.model, this.context);
                    this.dataset.create_function = function() {
                        return self.options.create_function.apply(null,
                                arguments).then(function(r) {
                ...
                setup_search_view : function(search_defaults) {
                    ...
                    this.searchview.on_search.add(function(domains, contexts,
                            groupbys) {
                        if (self.initial_ids) {
                            self.do_search(domains.concat([
                                    [ [ "id", "in", self.initial_ids ] ],
                                    self.domain ]), contexts, groupbys);
                            self.initial_ids = undefined;
                        } else {
                            self.do_search(domains.concat([ self.domain ]),
                                    contexts.concat(self.context), groupbys);
                        }
                    });
                    this.searchview.on_loaded
                            .add_last(function() {
                                self.view_list = new openerp.web.form.SelectCreateListView(
                                        self,
                                        self.dataset,
                                        false,
                                        _
                                ...
                                self.view_list.on_loaded
                                        .add_last(function() {
                                            var $buttons = self.view_list.$element
                                                    .find(".oe-actions");
                                            $buttons
                                                    .prepend(QWeb
                                                            .render("SelectCreatePopup.search.buttons"));
                                            var $cbutton = $buttons
                                            ...
                                            $sbutton
                                                    .click(function() {
                                                        self
                                                                .on_select_elements(self.selected_ids);
                                                        self.stop();
                                                    });
                                        });
                            });
                    this.searchview.appendTo($("#" + this.element_id
                            + "_search"));
                },
                ...
                create_row : function() {
                    var self = this;
                    var wdataset = new openerp.web.DataSetSearch(this,
                            this.model, this.context, this.domain);
                ...
                on_click_element : function(ids) {
                    ...
                    if (this.selected_ids.length > 0) {
                        ...
                        this.$element.find(
                                ".oe_selectcreatepopup-search-select").attr(
                                'disabled', "disabled");
                    }
                },
                new_object : function() {
                    ...
                    this.view_form = new openerp.web.FormView(this,
                            this.dataset, false, self.options.form_view_options);
                    if (this.options.alternative_form_view) {
                        this.view_form
                                .set_embedded_view(this.options.alternative_form_view);
                    }
                    this.view_form.appendTo(this.$element.find("#"
                            + this.element_id + "_view_form"));
                    this.view_form.on_loaded.add_last(function() {
                        var $buttons = self.view_form.$element
                                .find(".oe_form_buttons");
                        $buttons.html(QWeb.render(
                                "SelectCreatePopup.form.buttons", {
                                    widget : self
                                }));
                        var $nbutton = $buttons
                                .find(".oe_selectcreatepopup-form-save-new");
                        ...
                        var $nbutton = $buttons
                                .find(".oe_selectcreatepopup-form-save");
                        $nbutton.click(function() {
                            $.when(self.view_form.do_save()).then(function() {
                                self.view_form.reload_mutex.exec(function() {
                                    self.check_exit();
                        ...
                        var $cbutton = $buttons
                                .find(".oe_selectcreatepopup-form-close");
                        $cbutton.click(function() {
                            self.check_exit();
                        });
                    });
     ...
     * @extends openerp.web.OldWidget
     */
    openerp.web.form.FormOpenPopup = openerp.web.OldWidget
            .extend(/** @lends openerp.web.form.FormOpenPopup# */
            {
                template : "FormOpenPopup",
                /**
                 * options: - alternative_form_view - auto_write (default true) -
                 * read_function - parent_view - child_name - form_view_options -
                 * readonly
                 */
                ...
                start : function() {
                    this._super();
                    this.dataset = new openerp.web.form.FormOpenDataset(this,
                            this.model, this.context);
                ...
                setup_form_view : function() {
                    var self = this;
                    var FormClass = this.options.readonly ? openerp.web.views
                            .get_object('page') : openerp.web.views
                            .get_object('form');
                    this.view_form = new FormClass(this, this.dataset, false,
                            self.options.form_view_options);
                    if (this.options.alternative_form_view) {
                        this.view_form
                                .set_embedded_view(this.options.alternative_form_view);
                    }
                    this.view_form.appendTo(this.$element.find("#"
                            + this.element_id + "_view_form"));
                    this.view_form.on_loaded
                            .add_last(function() {
                                var $buttons = self.view_form.$element
                                        .find(".oe_form_buttons");
                                $buttons.html(QWeb
                                        .render("FormOpenPopup.form.buttons"));
                                var $nbutton = $buttons
                                        .find(".oe_formopenpopup-form-save");
                                $nbutton.click(function() {
                                ...
                                var $cbutton = $buttons
                                        .find(".oe_formopenpopup-form-close");
                                $cbutton.click(function() {
                                    self.stop();
                                });
    ...
    openerp.web.form.FieldReference = openerp.web.form.Field
            .extend({
                ...
                init : function(view, node) {
                    ...
                    this.fields = {};
                    this.fields_order = [];
                    this.selection = new openerp.web.form.FieldSelection(this,
                            {
                                attrs : {
                                    name : 'selection',
                                    widget : 'selection'
                    ...
                    this.selection.on_value_changed
                            .add_last(this.on_selection_changed);
                    this.m2o = new openerp.web.form.FieldMany2One(this, {
                        attrs : {
                            name : 'm2o',
                            widget : 'many2one'
                        }
                ...
                get_value : function() {
                    var model = this.selection.get_value(), id = this.m2o
                            .get_value();
    ...
    openerp.web.form.FieldBinary = openerp.web.form.Field
            .extend({
                init : function(view, node) {
                    this._super(view, node);
                ...
                start : function() {
                    this._super.apply(this, arguments);
                    this.$element.find('input.oe-binary-file').change(
                            this.on_file_change);
                    this.$element.find('button.oe-binary-file-save').click(
                            this.on_save_as);
                    this.$element.find('.oe-binary-file-clear').click(
                            this.on_clear);
                },
                ...
                on_file_change : function(e) {
                    // TODO: on modern browsers, we could directly read the file
                    // locally on client ready to be used on image cropper
                    // http://www.html5rocks.com/tutorials/file/dndfiles/
                    // http://deepliquid.com/projects/Jcrop/demos.php?demo=handler
                    window[this.iframe + '_callback'] = this.on_file_uploaded;
                    if ($(e.target).val() != '') {
                        this.$element.find(
                                'form.oe-binary-form input[name=session_id]')
    ...
    openerp.web.form.FieldBinaryFile = openerp.web.form.FieldBinary.extend({
        ...
        update_dom : function() {
            this._super.apply(this, arguments);
            this.$element.find('.oe-binary-file-set, .oe-binary-file-clear')
                    .toggle(!this.readonly);
            this.$element.find('input[type=text]').prop('readonly',
                    this.readonly);
        },
        set_value : function(value) {
            this._super.apply(this, arguments);
    ...
    openerp.web.form.FieldBinaryImage = openerp.web.form.FieldBinary.extend({
        ...
        set_value : function(value) {
            ...
            var url = '/web/binary/image?session_id=' + this.session.session_id
                    + '&model=' + this.view.dataset.model + '&id='
        ...
        on_file_uploaded_and_valid : function(size, name, content_type,
            ...
            this.value = file_base64;
            this.binary_value = true;
            this.$image.attr('src', 'data:' + (content_type || 'image/png')
                    + ';base64,' + file_base64);
            this.set_filename(name);
        },
        on_clear : function() {
    ...
    openerp.web.form.FieldStatus = openerp.web.form.Field
            .extend({
                template : "EmptyComponent",
                start : function() {
                    this._super();
                ...
                render_list : function() {
                    var self = this;
                    var shown = _.map(
                            ((this.node.attrs || {}).statusbar_visible || "")
                                    .split(","), function(x) {
                                return _.str.trim(x);
                            });
                    shown = _.select(shown, function(x) {
                        return x.length > 0;
                    });
                    ...
                    if (shown.length == 0) {
                        ...
                        this.to_show = _.select(this.field.selection, function(
                                x) {
                            return _.indexOf(shown, x[0]) !== -1
                                    || x[0] === self.selected_value;
                        });
                    }

                    var content = openerp.web.qweb.render(
                            "FieldStatus.content", {
                                widget : this,
                                _ : _
                            });
                    ...
                    var colors = JSON
                            .parse((this.node.attrs || {}).statusbar_colors
                                    || "{}");
                    var color = colors[this.selected_value];
                    if (color) {
                        var elem = this.$element
                                .find("li.oe-arrow-list-selected span");
                        elem.css("border-color", color);
                        if (this.check_white(color))
                            elem.css("color", "white");
                        elem = this.$element
                                .find("li.oe-arrow-list-selected .oe-arrow-list-before");
                        elem.css("border-left-color", "transparent");
                        elem = this.$element
                                .find("li.oe-arrow-list-selected .oe-arrow-list-after");
                ...
                check_white : function(color) {
                    ...
                    div.remove();
                    var res = /^\s*rgb\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)\s*$/
                            .exec(ncolor);
                    if (!res) {
                        return false;
                    }
                    var comps = [ parseInt(res[1]), parseInt(res[2]),
                            parseInt(res[3]) ];
                    var lum = comps[0] * 0.3 + comps[1] * 0.59 + comps[1]
                            * 0.11;
                    if (lum < 128) {
                        return true;
                    }
    ...
    openerp.web.form.WidgetHtml = openerp.web.form.Widget.extend({
        ...
        render_children : function(object, $into) {
            var self = this, fields = this.view.fields_view.fields;
            _(object.children).each(
                    function(child) {
                        if (typeof child === 'string') {
                            $into.text(child);
                        } else if (child.tag === 'field') {
                            $into.append(new (self.view.registry
                                    .get_object('frame'))(self.view, {
                                tag : 'ueule',
                                attrs : {},
                                children : [ child ]
                            }).render());
                            ...
                            var $child = $(document.createElement(child.tag))
                                    .attr(child.attrs).appendTo($into);
    ...
    openerp.web.form.widgets = new openerp.web.Registry({
        ...
        'bigint' : 'openerp.web.form.FieldFloat',
        ...
        // Alterado Integra
        //
        'priceformat' : 'openerp.web.form.PriceFormatWidget',
        'historicformat' : 'openerp.web.form.HistoricFormatWidget',
        'planocontaformat' : 'openerp.web.form.PlanoContaFormatWidget',
        'onlyedit' : 'openerp.web.form.WidgetOnlyEdit',
        'fieldreadonly' : 'openerp.web.form.FieldReadOnlyWidget'
    });
