    ...
    def enviar_nfse_barueri(self, cr, uid, ids, context={}):
        ...
        for doc_obj in self.browse(cr, uid, ids):
            ...
            if doc_obj.situacao == 'cancelada':
                ...
                rps_registro2.numero_rps = doc_obj.numero or -1
                
            rps_registro2.data_emissao_rps =  data_hora_horario_brasilia(parse_datetime(parse_datetime(doc_obj.data_emissao_rps)))
            rps_registro2.hora_rps =  data_hora_horario_brasilia(parse_datetime(parse_datetime(doc_obj.data_emissao_rps)))
            ...
            rps_registro2.codigo_servico_prestado = 0
            ...
            for item_obj in doc_obj.documentoitem_ids:
                rps_registro2.quantidade_servico_prestado = D(str(item_obj.quantidade[0])).quantize(D('0.01'))
                rps_registro2.valor_servico_prestado = D(str(item_obj.vr_servico[0])).quantize(D('0.01'))
                
            rps_registro2.valor_total_retencoes = D(str(doc_obj.vr_produtos)).quantize(D('0.01'))      
            ...
            rps_registro9.numero_total_linha = len(arquivo_texto.strip())        
