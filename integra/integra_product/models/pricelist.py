...
class product_pricelist(osv.Model):
    ...
    _columns = {
        'name': fields.char('Pricelist Name',size=64, required=True, translate=False),
        ...
        'ignora_impostos': fields.boolean(u'Ignora markup dos impostos na venda?'),
        'desconto_maximo': fields.float(u'Porcentagem máxima de desconto'),
    }
        ...
        for product_id, qty, partner in products_by_qty_by_partner:
            for pricelist_id in pricelist_ids:
                ...
                for item_obj in self.pool.get('product.pricelist.item').browse(cr, 1, item_ids):
                    if item_obj.base == OUTRA_LISTA_PRECO:
                        if not item_obj.base_pricelist_id:
                            ...
                            preco = price_tmp[item_obj.base_pricelist_id.id]
                            vr_unitario_base = price_tmp['vr_unitario_base'][item_obj.base_pricelist_id.id]
                            precos_originais[product_id] = vr_unitario_base
                            ...
                            price = currency_pool.compute(cr, uid, currency_ids[0], item_obj.base_pricelist_id.currency_id.id, preco, round=False)
                            price = D(price)
                        ...
                        precos_originais[product_id] = D(price or 0)
                        ...
                        #
                        # Quando for o preço de venda informado, o preço de custo também é o informado
                        #
                        if price_type.field == 'list_price' or price_type == 'standard_price':
                            preco_original = product_pool.price_get(cr, uid, [product_id], 'standard_price', context=context)[product_id]
                            precos_originais[product_id] = D(preco_original or 0)
                ...
                if price:
                    ...
                    results['vr_unitario_base'] = precos_originais[product_id]
    ...
    def price_get(self, cr, uid, ids, prod_id, qty, partner=None, context=None):
        ...
        res.update({'vr_unitario_base': {ids[-1]: res_multi.get('vr_unitario_base', ids[-1])}})
        return res
...
class product_pricelist_item(osv.Model):
    ...
    _columns = {
        ...
        'margem': fields.float(u'Margem', digits=(18,4)),
    ...
    def atualiza_regras(self, cr, uid, ids, context={}):
        for item_obj in self.browse(cr, uid, ids, context=context):
            margem = D(0)

            for regra_obj in item_obj.subitem_ids:
                margem += D(regra_obj.multiplica or 0) / D(regra_obj.divide or 1)

            item_obj.write({'margem': margem * 100, 'price_discount': margem - 1})
