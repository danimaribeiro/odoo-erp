...
class sped_documento(osv.Model):
    ...
    def _lista_vencimentos(self, cr, uid, payment_term_obj, vr_fatura, data_emissao, context={}):
        return payment_term_obj.compute(vr_fatura, date_ref=data_emissao)

    def onchange_payment_term(self, cr, uid, ids, payment_term_id, vr_fatura, vr_nf, data_emissao, duplicata_ids, context={}):
        valores = {}
        ...
        if ids:
            doc_obj = self.pool.get('sped.documento').browse(cr, uid, ids[0])

            if getattr(doc_obj, 'sale_order_id', False):
                context['sale_obj'] = doc_obj.sale_order_id

        lista_vencimentos = self.pool.get('sped.documento')._lista_vencimentos(cr, uid, payment_term_obj, vr_fatura, data_emissao, context=context)
        ...
        if lista_vencimentos:
            ...
            for data, valor in lista_vencimentos:
                ...
                if getattr(payment_term_obj, 'bloqueia_pagamento', False) and parcela == len(lista_vencimentos):
                    dados['pagamento_bloqueado'] = True

            valores['duplicata_ids'] = duplicata_ids

        return retorno

    def botao_regera_duplicatas(self, cr, uid, ids, context={}):
        for doc_obj in self.pool.get('sped.documento').browse(cr, uid, ids):
            if not doc_obj.payment_term_id:
                doc_obj.write({'recalculo': int(random.random() * 100000000)})

            else:
                valores = doc_obj.onchange_payment_term(doc_obj.payment_term_id.id, doc_obj.vr_fatura, doc_obj.vr_nf,  doc_obj.data_emissao, [])
                doc_obj.write(valores['value'])

        for doc_obj in self.pool.get('sped.documento').browse(cr, uid, ids):
            soma_duplicatas = D(0)
            fatura = D(doc_obj.vr_fatura or 0)
            fatura = fatura.quantize(D('0.01'))

            for dup_obj in doc_obj.duplicata_ids:
                soma_duplicatas += D(dup_obj.valor or 0).quantize(D('0.01'))

            if soma_duplicatas != fatura:
                raise osv.except_osv(u'Aviso!', u'A soma do(s) valor(es) da(s) duplicata(s): R$ {soma} não bate com o valor da fatura: R$ {fatura}!'.format(soma=formata_valor(soma_duplicatas), fatura=formata_valor(fatura)))

            doc_obj.gera_financeiro()
    ...
    def create(self, cr, uid, dados, context={}):
        ...
        self.pool.get('sped.documento').gera_financeiro(cr, uid, [res])
    ...
    def write(self, cr, uid, ids, dados, context={}):
        ...
        if dados and ('recalculo' not in dados) and ('cce_ids' not in dados):
    ...
    def gera_financeiro(self, cr, uid, ids):
        ...
        for doc_obj in self.browse(cr, uid, ids):
            if not doc_obj.finan_conta_id:
                continue

            if doc_obj.finan_lancamento_id:
                dados = {}

                if doc_obj.finan_lancamento_id.provisionado:
                    dados['provisionado'] = False

                if not doc_obj.finan_lancamento_id.sped_documento_id:
                    dados['sped_documento_id'] = doc_obj.id

                if len(dados):
                    lancamento_pool.write(cr, uid, [doc_obj.finan_lancamento_id.id], dados)

            elif (doc_obj.numero > 0):
                ...
                if doc_obj.purchase_order_id:
                    dados['historico'] = doc_obj.purchase_order_id.notes

                if doc_obj.res_partner_bank_id:
                    dados['res_partner_bank_id'] = doc_obj.res_partner_bank_id.id
                    dados['sugestao_bank_id'] = doc_obj.res_partner_bank_id.id
                ...
                if  doc_obj.operacao_id.formapagamento_id:
                    dados['formapagamento_id'] = doc_obj.operacao_id.formapagamento_id.id
                ...
                for duplicata_obj in doc_obj.duplicata_ids:
                    ...
                    if dados['tipo'] == 'P' and getattr(duplicata_obj, 'pagamento_bloqueado', False):
                        dados['pagamento_bloqueado'] = True

                    #print('dados duplicata', dados)
                    ...
                    if lancamento_id:
                        ...
                        salva_rateio = [[5, False, False]]
                        nova_conta_id = None
                        for dic_rateio in dados_rateio:
                            if nova_conta_id is None and 'conta_id' in dic_rateio and dic_rateio['conta_id']:
                                nova_conta_id = dic_rateio['conta_id']

                            salva_rateio += [[0, lancamento_obj.id, dic_rateio]]

                        #print('nova_conta_id')
                        #print(lancamento_obj)
                        #print('salva_rateio')
                        #print(salva_rateio)
                        if nova_conta_id is None:
                            lancamento_obj.write({'rateio_ids': salva_rateio})
                        else:
                            lancamento_obj.write({'rateio_ids': salva_rateio, 'conta_id': nova_conta_id})
    ...
    def cancela_financeiro(self, cr, uid, ids):
        ...
        for doc_obj in self.browse(cr, uid, ids):
            #
            # Exclui as provisões de ordem de compra, quando houver
            #
            if getattr(doc_obj, 'purchase_order_id', False):
                try:
                    cr.execute('delete from finan_lancamento where purchase_order_id = {ordem_id} and provisionado = True;'.format(ordem_id=doc_obj.purchase_order_id.id))
                except:
                    pass
    ...
    def _dados_rateio(self, cr, uid, ids, context={}):
        ...
        for doc_obj in self.browse(cr, uid, ids):
            ...
            for item_obj in doc_obj.documentoitem_ids:
                item_obj.realiza_rateio(rateio=rateio, context=context)
