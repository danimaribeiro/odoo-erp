...
class importa_finan(osv.Model):
    ...
    _columns = {
        ...
        'nome_partner': fields.char(u'Nome parceiro', size=60, select=True),
        ...
        'conta_antiga': fields.char(u'Conta antiga', size=20, select=True),
        'conta_id': fields.many2one('finan.conta', u'Conta Financeira', select=True, ondelete='restrict'),
        'documento_antigo': fields.char(u'Documento antigo', size=20, select=True),
        ...
        'carteira_antiga': fields.char(u'Carteira antiga', size=20, select=True),
    ...
    def replica_parceiro(self, cr, uid, ids, context={}):
        ...
        for if_obj in self.browse(cr, uid, ids):
            ...
            if if_obj.cnpj_partner == '000.000.000-00':
                cr.execute(u"update importa_finan set partner_id = {partner_id} where cnpj_partner = '{cnpj_partner}' and nome_partner = '{nome_partner}' and partner_id is null;".format(partner_id=if_obj.partner_id.id, cnpj_partner=if_obj.cnpj_partner, nome_partner=if_obj.nome_partner))
    ...
    def replica_documento(self, cr, uid, ids, context={}):
        if not ids:
            return
        ...
        for if_obj in self.browse(cr, uid, ids):
            if if_obj.documento_antigo in ja_feitos:
                continue

            if not if_obj.documento_id:
                continue

            cr.execute("update importa_finan set documento_id = {documento_id} where documento_antigo = '{documento_antigo}' and documento_id is null;".format(documento_id=if_obj.documento_id.id, documento_antigo=if_obj.documento_antigo))
            ja_feitos.append(if_obj.documento_antigo)

        return True

    def replica_carteira(self, cr, uid, ids, context={}):
        if not ids:
            return
        ...
        for if_obj in self.browse(cr, uid, ids):
            if if_obj.carteira_antiga in ja_feitos:
                continue

            if not if_obj.carteira_id:
                continue

            if not if_obj.carteira_antiga:
                continue

            cr.execute("update importa_finan set carteira_id = {carteira_id} where carteira_antiga = '{carteira_antiga}' and carteira_id is null and cnpj_empresa = '{cnpj_empresa}';".format(carteira_id=if_obj.carteira_id.id, carteira_antiga=if_obj.carteira_antiga, cnpj_empresa=if_obj.cnpj_empresa))
            ja_feitos.append(if_obj.carteira_antiga)
