...
class sale_report(osv.osv):
    ...
    _columns = {
        ...
        'vr_mensalidade_servico': fields.float(u'Mensalidade serviço', readonly=True),
        'vr_mensalidade_locacao': fields.float(u'Mensalidade locação', readonly=True),
        ...
        cr.execute("""
            ...
            create or replace view sale_report as (
                select
                    ...
                    case
                      when s.state = 'manual' and s.orcamento_aprovado = 'venda' then s.vr_total_margem_desconto
                      when s.state = 'draft' then s.vr_total_margem_desconto
                      else 0
                    end as price_total,

                    case
                      when s.state = 'manual' and s.orcamento_aprovado = 'locacao' then coalesce((select sum(rc.vr_mensal) from orcamento_orcamento_locacao rc where rc.sale_order_id = s.id and rc.orcamento_categoria_id = 9), 0)
                      when s.state = 'draft' then coalesce((select sum(rc.vr_mensal) from orcamento_orcamento_locacao rc where rc.sale_order_id = s.id and rc.orcamento_categoria_id = 9), 0)
                      else 0
                    end as vr_mensalidade_servico,

                    case
                      when s.state = 'manual' and s.orcamento_aprovado = 'locacao' then coalesce((select sum(rc.vr_mensal) from orcamento_orcamento_locacao rc where rc.sale_order_id = s.id and rc.orcamento_categoria_id != 9), 0)
                      when s.state = 'draft' then coalesce((select sum(rc.vr_mensal) from orcamento_orcamento_locacao rc where rc.sale_order_id = s.id and rc.orcamento_categoria_id != 9), 0)
                      else 0
                    end as vr_mensalidade_locacao,
