...
class sesmt_risco(osv.Model):
    _name = 'sesmt.risco'
    _description = u'Riscos'
    _rec_name = 'descricao'
    _order = 'descricao'
    ...
    _columns = {
        'tipo': fields.selection(TIPO_RISCO, u'Tipo', required=True, select=True),
        'data_inicial': fields.date(u'Data inicial', required=True, select=True),
        'data_final': fields.date(u'Data final', select=True),
        'descricao': fields.text(u'Descrição', required=True, select=True),
        'recomendacao_id': fields.many2one('sesmt.recomendacao', u'Recomendação', required=True),
    }
