...
ESTRUTURA = [
    ...
    ('DRE', u'Demonstração do Resultado do Exercício'),
    ...
    ('GERENCIAL_1', u'Gerencial 1'),  # A vontade do cliente
]


class ecd_estrutura_demonstrativo(osv.Model):
    _name = 'ecd.estrutura.demonstrativo'
    _description = u'Estrutura dos demonstrativos'
    ...
    def monta_codigo(self, cr, uid, id):
        ...
        if conta_obj.parent_id:
            cr.execute('select max(codigo) from ecd_estrutura_demonstrativo where parent_id = ' + str(conta_obj.parent_id.id) + ';')
            res = cr.fetchall()[0][0]
    ...
    _columns = {
        ...
        'parent_id': fields.many2one('ecd.estrutura.demonstrativo', u'Conta pai', select=True, ondelete='cascade'),
        'contas_filhas_ids': fields.one2many('ecd.estrutura.demonstrativo', 'parent_id', string=u'Contas filhas'),
        ...
        'company_ids': fields.many2many('res.company', 'ecd_estrutura_demonstrativo_company', 'estrutura_id', 'company_id', u'Empresas'),
        'conta_ids': fields.many2many('finan.conta', 'ecd_estrutura_demonstrativo_finan_conta', 'estrutura_id', 'finan_conta_id', u'Contas financeiras'),
        'centrocusto_ids': fields.many2many('finan.centrocusto', 'ecd_estrutura_demonstrativo_centrocusto', 'estrutura_id', 'centrocusto_id', u'Centros de custo'),
        'hr_department_ids': fields.many2many('hr.department', 'ecd_estrutura_demonstrativo_departamento', 'estrutura_id', 'department_id', u'Departamentos/postos'),
        'partner_category_ids': fields.many2many('res.partner.category', 'ecd_estrutura_demonstrativo_categoria_cliente', 'estrutura_id', 'categoria_id', u'Categorias de clientes'),
        ...
        'resumida_soma': fields.many2many('ecd.estrutura.demonstrativo', 'ecd_estrutura_demonstrativo_soma', 'estrutura_id', 'conta_soma_id', u'Contas a somar'),
        'resumida_subtrai': fields.many2many('ecd.estrutura.demonstrativo', 'ecd_estrutura_demonstrativo_subtrai', 'estrutura_id', 'conta_subtrai_id', u'Contas a subtrair'),
    }
    ...
    def _verifica_recursiva(self, cr, uid, ids, context={}):
        ...
        while len(ids):
            cr.execute('select distinct parent_id from ecd_estrutura_demonstrativo where id in %s', (tuple(ids),))
    ...
    def create(self, cr, uid, vals, context={}):
        ...
        res = super(ecd_estrutura_demonstrativo, self).create(cr, uid, vals, context)
    ...
    def write(self, cr, uid, ids, vals, context={}):
        ...
        res = super(ecd_estrutura_demonstrativo, self).write(cr, uid, ids, vals, context=context)
        return res
