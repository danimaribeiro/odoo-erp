...
class mail_server(orm.Model):
    _name = 'ir.mail_server'
    _inherit = 'ir.mail_server'

    _columns = {
        'limite_diario': fields.integer(u'Limite diário para envio de mensagens'),
    }
    ...
    def send_email(self, cr, uid, message, mail_server_id=None, smtp_server=None, smtp_port=None,
        ...
        if mail_server:
            ...
            #
            # Trata o limite de mensagens por dia
            #
            if mail_server.limite_diario:
                message_pool = self.pool.get('mail.message')
                message_ids = message_pool.search(cr, 1, [('state', 'in', ('outgoing', 'sent', 'exception')), ('date', '>=', str(fields.date.today()) + ' 00:00:00'), ('date', '<=', str(fields.date.today()) + ' 23:59:59')])

                if len(message_ids):
                    conta = len(message_ids) / mail_server.limite_diario

                    if conta:
                        partes = smtp_user.split('@')
                        smtp_user = partes[0] + '_' + str(conta) + '@' + partes[1]
                        smtp_from = smtp_user
        ...
        if True:
            ...
            try:
                print(smtp_server, smtp_port, smtp_user, smtp_password, smtp_encryption, smtp_debug)
