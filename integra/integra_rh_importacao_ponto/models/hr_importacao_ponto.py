...
class hr_importacao_ponto(osv.Model):
    ...
    def processar_retorno(self, cr, uid, id, context=None):
        ...
        for linha in arquivo.readlines():
            ...
            hora = D(linha[17:22])
            minutos = D(linha[22:24]) / D(60.0)
            horas = hora + minutos
            print(linha, linha[17:22], linha[22:24], hora, minutos, horas)
            ...
            if not contract_id or len(rule_id) == 0:
                ...
                if len(rule_id) == 0:
                    texto += u'Rubrica não cadastrada! '
    ...
    def unlink(self, cr, uid, ids, context={}):
        if uid != 1:
            raise osv.except_osv(u'Erro!', u'Você não tem permissão de excluir o arquivo de importação do ponto!')

        super(hr_importacao_ponto, self).unlink(cr, uid, ids, context=context)
