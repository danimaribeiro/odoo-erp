...
from osv import osv
#from orcamento.models.orcamento_categoria import orcamento_categoria as orccat_original

CORPO_FORM = u'''
    <group string="{categoria_nome}" colspan="10">
        <field name="{nome_campo}" colspan="10" nolabel="1" widget="one2many_list" mode="tree,form" attrs="{{'readonly': [('state', '!=', 'draft')]}}" context="{{'default_veiculo_id': veiculo_id, 'veiculo_id': veiculo_id, 'default_modelo_id': modelo_id, 'modelo_id': modelo_id, 'default_product_ids': product_ids, 'product_ids': product_ids}}">
                ...
                <notebook>
                    <page string="Item">
                        <group colspan="4" col="5">
                            <field colspan="3"
                                   domain="[('orcamento_categoria_id.id', '=', {categoria_id}),'|',('qualquer_marca_modelo', '=', True),('id', 'in', product_ids[0][2])]" context="{{'default_orcamento_categoria_id': {categoria_id},                                    'partner_id':parent.partner_id, 'quantity':product_uom_qty, 'pricelist':parent.pricelist_id, 'shop':parent.shop_id, 'uom':product_uom, 'force_product_uom': True}}"
                                   name="product_id"
            ...
            <tree string="{categoria_nome}" editable="top">
                    ...
                    <field name="product_ids" invisible="1" />
                    <field
                           domain="[('orcamento_categoria_id.id', '=', {categoria_id}),'|',('qualquer_marca_modelo', '=', True),('id', 'in', product_ids[0][2])]" context="{{'default_orcamento_categoria_id': {categoria_id},
...
class orcamento_categoria(osv.Model):
    ...
    def ajusta_parametro_views(self):
        ...
        self.corpo_form_sale = CORPO_FORM

        print('ajustou view aqui')
