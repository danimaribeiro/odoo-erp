...
class sped_ecd_relatorio(osv.osv_memory):
    ...
    def gera_relatorio_razao_financeiro(self, cr, uid, ids, context={}):
        ...
        project_ids = []
        if len(rel_obj.project_ids) > 0:    
    
            descricao += u'Projetos: ' 
            for project_obj in rel_obj.project_ids:
        ...
        from (
            ...
            left join project_project ppr on ppr.id = lcr.project_id
        ...
        if len(project_ids) > 0:  
            sql += """
                and lcr.project_id in """ +  str(tuple(project_ids)).replace(',)', ')')
