...
class res_currency(orm.Model):
    ...
    _columns = {
        ...
        'symbol': fields.char(u'Símbolo', size=3),
    ...
    def atualiza_cotacao(self, cr, uid, ids=[], context={}):
        ...
        for moeda_obj in self.browse(cr, uid, ids):
            ...
            while data <= data_final:
                ...
                elif moeda_obj.codigo_ws_bacen == str(WS_BC_IGPM):
                    if data.day != 2:
                        data += relativedelta(days=+1)
                        continue

                    valor = cotacao_igpm_12_meses(certificado, data)

                    obs = u'Do mês: ' + formata_valor(cotacao_igpm(certificado, data))
...
class res_currency_rate(orm.Model):
    ...
    _columns = {
        ...
        'obs': fields.char(u'Obs.', size=100),
    }
