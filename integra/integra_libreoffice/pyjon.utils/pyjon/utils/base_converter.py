...
def decimal_to_n(
    ...
    if left_digits == 0:
        ...
        return decimal_to_n(
            left_digits, base_to, numerals
        ) + numerals[num % base_to]
...
def n_to_m(
    ...
    @param numerals_to: the list of symbols to use in the destination base
    @type numerals_to: string
    ...
    if not 2 <= base_to <= len(numerals_to):
        raise ValueError(
            "The base_to parameter must be between 2 and %s" % len(
                numerals_to
            )
        )
