...
def substitute(
        string, callback, default='', regex=r"\{([^{}]+?)\}", sub_if_none=True
):
    ...
    if not isinstance(default, six.string_types):
        raise ValueError(
            "The default value must be a basestring (py2), "
            "or str (py3), not a %s" % type(default)
        )
    ...
    for match in pattern.finditer(string):
        ...
        rep_is_base = isinstance(rep, six.string_types)
